...
| 快照与 WAL 资源在生产策略下保持有界 | `nosqlite/test_storage_page_basics.uya` 的 snapshot pressure/checkpoint policy；`nosqlite/test_storage_wal_runtime.uya` 的 checkpoint truncate 和 soft-limit checkpoint；`nosqlite/test_exec_runtime.uya` 的 hard pressure cleanup；`nosqlite/test_stress_runtime.uya` 的 repeated pressure cycle |
...
| 所有核心模块至少有单元测试 | `nosqlite/tests/README.md` 覆盖 core/doc/sql/plan/exec/storage 分组；`nosqlite/test_stress_runtime.uya` 覆盖跨 exec/storage 的压力路径 |
...
`verify_definition_of_done.sh` 会串行运行全部当前可执行 Uya 测试入口，包括 `nosqlite/test_stress_runtime.uya` 压力门，并执行 Phase 12.5 / Phase 14 的辅助验证脚本。脚本还会检查 `docs/nosqlite-benchmark-v0.json` 中 DoD 相关 benchmark case 的 `floor_status`。
