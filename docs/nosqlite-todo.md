...
以下为持续执行规则，不作为一次性交付 checkbox 计入完成率。
...
- [x] 产出 v0 实测性能基线
- [x] 用 v0 实测数据替换第 18 节预算数字
- [x] 统计并输出 `p50/p95/p99`
- [x] 统计并输出 `docs/s`、`MiB/s`
- [x] 统计并输出 `peak_memory`
- [x] 验证所有 v1 benchmark 至少达到 `floor`
...
- [x] 记录“长查询 + 并发提交”相对基线保持率的 `target/stretch` 达成情况
...
- [x] 先写失败测试：字段不存在、类型不匹配、schema 与 SQL 不一致
- [x] 定义静态 schema 描述格式
- [x] 设计 `typed_sql(sql, schema)` 接口
- [x] 只做静态字段与类型校验
- [x] 不把编译期 schema 当成运行时事实源
- [x] 为宏路径增加回归测试
- [x] 输出清晰的编译期错误信息（当前由 Phase 14 静态验证脚本承载，等待 Uya 条件化 `@mc_error` 后迁回宏期）

## Definition of Done

- [x] 新建数据库、插入、查询、重启恢复全链路可跑通
- [x] `_id` 主键查找明显快于全表扫描
- [x] 未提交事务不会在恢复后可见
- [x] 已提交事务在断电恢复后可见
- [x] 长查询执行期间提交不会打断已有读者
- [x] 同时具备 `db_query` 和 `db_query_cursor`
- [x] 点查、顺扫、写入、恢复四类 benchmark 至少达到设计文档第 18 节 `floor`
- [x] 大整数与精确小数无静默降精度
- [x] 快照与 WAL 资源在生产策略下保持有界
- [x] 文件格式升级/拒绝打开不兼容版本行为经过验证
- [x] 核心资源类型完成 `drop` 封装
- [x] 核心错误路径完成 `errdefer` 回滚
- [x] 锁与 pin 完成 guard 化
- [x] 每个完成的能力项都能对应到明确的测试文件或测试组
- [x] 所有核心模块至少有单元测试
- [x] 至少有一组故障恢复测试
- [x] 文档与示例可以独立指导使用
