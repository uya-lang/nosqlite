...
第 18 节中的性能数字已经替换为第一版可运行原型的 v0 实测基线。

但当前基线仍有明确边界：

- 使用的是缩小版数据集，而不是最终 `100_000` 文档参考数据集
- 仍然受当前单页 collection 原型容量限制
- 因此它代表“原型阶段回归门槛”，还不是最终参考环境 SLO

它们当前的用途是：

- 为当前原型提供第一版实测回归门槛
- 固定 `floor / target / stretch` 的实际观测值
- 为后续扩容到 `100_000` 文档后的再标定提供起点

它们不是：

- 已校准的客户对外 SLO

在支持完整参考数据集之后，第 18 节仍必须再走一轮重标定：

1. 先产出 benchmark harness 与固定数据集
2. 在参考环境上跑出 v0 实测基线
3. 用新的参考环境基线替换当前缩小版原型基线
...
第 18 节现已不再使用“工程预算数字”，而是改成基于 `docs/nosqlite-benchmark-v0.*` 的 v0 原型实测基线。

当前约束必须明确写清：

- 这是 2026-04-23 的 v0 原型基线
- 当前数据集仍是缩小版：`3` 文档、平均约 `1 KiB`
- 当前实现仍受单页 collection 布局限制，因此这些数字是“原型阶段门槛”，不是最终参考环境 SLO
- 一旦存储原型支持 `100_000` 文档参考数据集，第 18 节必须再次按实测重标定
...
当前基线对应 `warm-read / primary_lookup`，查询形态仍为 `SELECT _id, $.name FROM users WHERE _id = ? LIMIT 1`。

| 指标 | floor | target | stretch |
|------|-------|--------|---------|
| `db_query` 单行物化返回 `p50` | `<= 23_000 us` | `<= 19_000 us` | `<= 16_000 us` |
| `db_query` 单行物化返回 `p95` | `<= 25_000 us` | `<= 20_000 us` | `<= 18_000 us` |

### 18.2 顺序扫描与过滤

当前基线对应 `warm-read / seq_scan_filter`，查询形态仍为 `SELECT _id FROM users WHERE $.age >= 18`。

| 指标 | floor | target | stretch |
|------|-------|--------|---------|
| 顺扫过滤吞吐 | `>= 140 docs/s` | `>= 160 docs/s` | `>= 180 docs/s` |
| 等效字节吞吐 | `>= 0.14 MiB/s` | `>= 0.16 MiB/s` | `>= 0.18 MiB/s` |
| 峰值运行内存 | `<= 32_000 KiB` | `<= 30_000 KiB` | `<= 28_000 KiB` |

### 18.3 路径提取

独立“纯路径提取”微基准尚未从查询基准中剥离。

当前要求改成：

- 仍然必须保证数值路径求值与执行器比较语义一致
- 在补出 standalone path microbench 前，这一项不单独作为性能出货门槛

### 18.4 持久化写入

当前基线对应 `durable-write / durable_insert`，仍是 auto-commit 单条插入。

| 指标 | floor | target | stretch |
|------|-------|--------|---------|
| sustained throughput | `>= 45 rows/s` | `>= 48 rows/s` | `>= 55 rows/s` |
| 单条提交 `p95` | `<= 26_000 us` | `<= 25_000 us` | `<= 22_000 us` |

批量事务插入基准仍待单页 collection 限制解除后再单独补齐。
...
| 指标 | floor | target | stretch |
|------|-------|--------|---------|
| reopen+recovery `p95` | `<= 110_000 us` | `<= 100_000 us` | `<= 90_000 us` |
| 恢复吞吐 | `>= 30 docs/s` | `>= 32 docs/s` | `>= 35 docs/s` |

### 18.6 并发读写稳定性

当前基线对应 `durable-write / long_query_concurrent_commit`，统计“存在长查询读者时写入吞吐相对无读者基线”的保持率。

| 指标 | floor | target | stretch |
|------|-------|--------|---------|
| 相对无读者基线保持率 `p50` | `>= 90%` | `>= 100%` | `>= 110%` |

### 18.7 内存上界

当前 v0 原型各 case 的实测 `peak_memory` 大致落在 `28 MiB ~ 31 MiB` 区间。

在当前原型阶段，内存门槛按各基准 case 自带的 `peak_memory_kib` 判定为准：

- 点查 / 顺扫：`floor <= 32_000 KiB`，`target <= 30_000 KiB`
- durable insert：`floor <= 30_000 KiB`，`target <= 29_000 KiB`
- recovery open：`floor <= 32_000 KiB`，`target <= 31_000 KiB`
